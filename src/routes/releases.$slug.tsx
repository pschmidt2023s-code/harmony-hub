...
const findRelease = (content: SiteContent | undefined, slug: string): Release | null =>
  content ? (publishedReleases(content.releases).find((r) => slugify(r.title) === slug) ?? null) : null;
...
export const Route = createFileRoute("/releases/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    const title = `TAYO — ${release.title}`;
    const description =
...
function ReleaseLanding({ release, content }: { release: Release; content: SiteContent }) {
  ...
  const services = streamingServices(tracks);
  const credits = releaseCredits(tracks);
